-- ==================================================
-- THE HACKER V6 - ULTIMATE EDITION (FIXED)
-- YouTube: @Hacker666-00
-- PC/Mobile Buttons FUNKTIONIEREN JETZT!
-- ==================================================
...
local function getGuiSize(device)
    if device == "Auto" then
...
local guiSizes = getGuiSize(Settings.Device)
...
HomeSettingsBtn.Name = "HomeSettingsBtn"
...
MainSettingsBtn.Name = "MainSettingsBtn"
...
for i, tabName in ipairs(tabs) do
    ...
    button.TextSize = guiSizes.TabSize
    button.Text = tabName
    button.Parent = Tabs
...
local function scanAllTools()
    scannedTools = {}
    local locations = {
        Workspace, game:GetService("StarterPack"), game:GetService("StarterGui"),
        game:GetService("ReplicatedStorage"), game:GetService("ServerStorage"),
        game:GetService("Lighting"), game:GetService("ReplicatedFirst")
    }
...
local function createAISection()
    ...
    aiMainFrame.Size = UDim2.new(1, -20, 0, 500)
    ...
    aiInput.TextSize = 14
    aiInput.PlaceholderText = "Was soll ich erstellen? (Auto, Haus, Waffe, Baum, Mensch, Tier, Möbel, Fahrzeug...)"
    ...
    presetsFrame.Size = UDim2.new(1, -40, 0, 150)
    presetsFrame.Position = UDim2.new(0, 20, 0, 190)
    ...
    local presets = {
        ...
        {"🪑 Stuhl",9}, {"🛏️ Bett",10}, {"🚁 Helikopter",11}, {"🚤 Boot",12},
        {"⚔️ Schwert",13}, {"🛡️ Schild",14}, {"🍔 Essen",15}, {"💎 Diamant",16}
    }
    ...
    resultFrame.Position = UDim2.new(0, 20, 0, 350)
    ...
    aiCreateButton.MouseButton1Click:Connect(function()
        ...
        if cmd:find("auto") or cmd:find("car") then
            ...
            car.Parent = Workspace; resultText.Text = "✅ Auto erstellt!"
        elseif cmd:find("haus") or cmd:find("house") then
            ...
            house.Parent = Workspace; resultText.Text = "✅ Haus erstellt!"
        elseif cmd:find("waffe") or cmd:find("gun") then
            ...
            tool.Parent = player.Backpack; resultText.Text = "✅ Waffe erhalten!"
        else
...
local function showMainTab()
    ...
    local buttons = {
        ...
        {"⚙️ Einstellungen öffnen", Color3.fromRGB(150, 150, 150)},
    ...
    for _, btnData in ipairs(buttons) do
        ...
        elseif btnData[1]:find("Einstellungen") then
            button.MouseButton1Click:Connect(function() settingsMenu.Visible = true end)
...
local function showShopTab()
    ...
    if #items == 0 then
        items = {"Robux", "RBX", "Currency", "Coins", "Gems", "Diamonds", "Gold", "Points"}
    end
...
deviceLabel.Parent = settingsMenu
...
deviceDropdown.Parent = settingsMenu
...
deviceDropdown.MouseButton1Click:Connect(function()
    for _, btn in pairs(deviceDropdown:GetChildren()) do
        if btn:IsA("TextButton") then btn.Visible = not btn.Visible end
    end
end)
...
local colors = {
    Color3.fromRGB(255,0,0), Color3.fromRGB(0,255,0), Color3.fromRGB(0,255,255),
...
for i, color in ipairs(colors) do
    ...
    colorBtn.MouseButton1Click:Connect(function()
        ...
        for _, btn in pairs(colorFrame:GetChildren()) do
            if btn:IsA("TextButton") and btn ~= colorDisplay then btn.Visible = false end
        end
    end)
    colorX = colorX + 35
end
...
colorFrame.MouseButton1Click:Connect(function()
    for _, btn in pairs(colorFrame:GetChildren()) do
        if btn:IsA("TextButton") and btn ~= colorDisplay then btn.Visible = not btn.Visible end
    end
end)
contentY = contentY + 50
...
PcButton.MouseButton1Click:Connect(function()
    print("PC Button clicked!") -- Debug
    Settings.Device = "PC"
    guiSizes = getGuiSize("PC")
    ...
    for i, tabName in ipairs(tabs) do
        tabButtons[tabName].TextSize = guiSizes.TabSize
    end
...
MobileButton.MouseButton1Click:Connect(function()
    print("Mobile Button clicked!") -- Debug
    Settings.Device = "Mobile"
    guiSizes = getGuiSize("Mobile")
    HomeFrame.Visible = false
    MainFrame.Visible = true
    MainFrame.Size = guiSizes.Main
    MainFrame.Position = UDim2.new(0.5, -guiSizes.Main.X.Offset/2, 0.5, -guiSizes.Main.Y.Offset/2)
